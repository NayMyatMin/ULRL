...
def contains_substring(state, substring):
    """
    Check if the substring exists in any string within the state.
    """
    if isinstance(state, dict):
        return any(contains_substring(value, substring) for value in state.values())
    elif isinstance(state, list):
        return any(contains_substring(item, substring) for item in state)
    elif isinstance(state, str):
        return substring in state
    return False

def replace_substring_in_state(state, old_substring, new_substring):
    """
    Recursively replace the old_substring with new_substring in all string values of the given state dictionary.
    """
    if isinstance(state, dict):
        for key, value in state.items():
            state[key] = replace_substring_in_state(value, old_substring, new_substring)
    elif isinstance(state, list):
        for i, item in enumerate(state):
            state[i] = replace_substring_in_state(item, old_substring, new_substring)
    elif isinstance(state, str):
        state = state.replace(old_substring, new_substring)
    return state
...
def load_attack_result(
    ...
    if all(key in load_file for key in ['model_name',
        ...
        state_file = load_file['bd_test']
        old_substring = "/workspace2/chenhongrui/bdzoo2_t914"
        old_substring1 = "/workspace/chenhongrui/bdzoo2_t914"
        if contains_substring(state_file, old_substring):
            new_substring = os.getcwd()
            state_file = replace_substring_in_state(
                state_file, 
                old_substring, 
                new_substring
            )
        if contains_substring(state_file, old_substring1):
            new_substring = os.getcwd()
            state_file = replace_substring_in_state(
                state_file, 
                old_substring1, 
