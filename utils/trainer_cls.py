...
############################################################################################################
#############################            ADDITION FOR UNLEARNING           #################################
############################################################################################################
        
class UnlearningTrainer(BackdoorModelTrainer):
    ...
    def test_on_mix(self):
        ...
        logging.info(f'Before Unlearning | '
                     f'Clean Test Loss: {clean_test_loss_avg_over_batch:.4f} | Clean Test ACC: {test_acc:.4f} '
                     f'| Poison Test Loss: {bd_test_loss_avg_over_batch:.4f} | Poison Test ACC: {test_asr:.4f}')

    def one_forward_backward_unlearn(self, x, labels, device, verbose=0):
        # Maximize the loss on the clean data
        self.model.train()
        self.model.to(device, non_blocking=self.non_blocking)
        ...
        with torch.cuda.amp.autocast(enabled=self.amp):
            log_probs = self.model(x)
            loss = self.criterion(log_probs, labels.long())
        self.scaler.scale(-loss).backward() # <== maximize the loss

        # Apply gradient clipping before optimizer step
        if self.scaler.is_enabled():
            # If using mixed precision, unscale gradients first
    ...
    def train_one_epoch_on_mix(self, verbose=0):
        ...
        for batch_idx in range(self.batch_num_per_epoch):
            x, labels, original_index, poison_indicator, original_targets  = self.get_one_batch()
            one_batch_loss, batch_predict = self.one_forward_backward_unlearn(x, labels, self.device, verbose)
            batch_loss_list.append(one_batch_loss)
    ...
    def train_with_test_each_epoch_on_mix(self,
        ...
        # Test on clean and backdoor data before unlearning
        self.test_on_mix()
        ...
        for epoch in range(total_epoch_num):
            ...
            if train_mix_acc <= self.clean_threshold:
                print(f"Early stopping at epoch {epoch} as train accuracy {train_mix_acc} is below threshold {self.clean_threshold}.")
                break 
    ...
    def plot_loss(
            ...
            clean_test_loss_list : list,
            bd_test_loss_list : list,
            save_file_name="unlearn_loss_metric_plots",
          ):
    ...
    def plot_acc_like_metric(self,
                 ...
                 test_asr_list: list,
                 test_ra_list: list,
                 save_file_name="unlearn_acc_like_metric_plots",
             ):
...
############################################################################################################
#############################            ADDITION FOR RELEARNING           #################################
############################################################################################################

class RelearningTrainer(BackdoorModelTrainer):

    def __init__(self, model, linear_name, neuron_indices, weight_mat_ori, relearn_alpha):
        super().__init__(model)
    ...
    def test_on_mix(self):
        clean_metrics, \
        _, _ = self.test_given_dataloader(self.test_dataloader_dict["clean_test_dataloader"], verbose=1)
        ...
        bd_test_epoch_predict_list, \
        bd_test_epoch_label_list, \
        _, _, _ = self.test_given_dataloader_on_mix(self.test_dataloader_dict["bd_test_dataloader"], verbose=1)

        bd_test_loss_avg_over_batch = bd_metrics["test_loss_avg_over_batch"]
        test_asr = all_acc(bd_test_epoch_predict_list, bd_test_epoch_label_list)

        logging.info(f'After Relearning | '
                     f'Clean Test Loss: {clean_test_loss_avg_over_batch:.2f} | Clean Test ACC: {test_acc:.4f} '
                     f'| Poison Test Loss: {bd_test_loss_avg_over_batch:.2f} | Poison Test ACC: {test_asr:.4f}')
    ...
    def one_forward_backward(self, x, labels, device, verbose=0):
        ...
        x, labels = x.to(device, non_blocking=self.non_blocking), labels.to(device, non_blocking=self.non_blocking)
        with torch.cuda.amp.autocast(enabled=self.amp):
            classification_loss = self.criterion(self.model(x), labels.long())
            neuron_weights = getattr(self.model, self.linear_name).weight[self.neuron_indices, :]
            cos_sim = torch.nn.functional.cosine_similarity(neuron_weights, self.weight_mat_ori, dim=1)
            loss = classification_loss + self.relearn_alpha * torch.sum(cos_sim)
        ...
        if verbose == 1:
            batch_predict = torch.max(self.model(x), -1)[1].detach().clone().cpu()
            return batch_loss, batch_predict
    ...
    def plot_loss(
            ...
            clean_test_loss_list : list,
            bd_test_loss_list : list,
            save_file_name="relearn_loss_metric_plots",
          ):
    ...
    def plot_acc_like_metric(self,
                 ...
                 test_asr_list: list,
                 test_ra_list: list,
                 save_file_name="relearn_acc_like_metric_plots",
             ):
